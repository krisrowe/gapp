...
When `auth.enabled` is set in `gapp.yaml`, gapp injects an ASGI wrapper (`gapp-run`) at deploy time. The wrapper sits between the client and the solution app, handling:

- **Client authentication** — validates JWTs (PATs) from `Authorization: Bearer` header or `?token=` query param
- **Credential lookup** — maps email hash to a credential file in GCS (mounted via FUSE at `/mnt/auth/`)
- **Strategy dispatch** — `bearer` (passthrough) or `google_oauth2` (refresh + cache)
- **Header rewrite** — replaces the client's JWT with the real upstream credential before forwarding

Solutions receive a standard `Authorization: Bearer <upstream-token>` header. No auth-aware code needed.

### Two Packages, Zero Overlap

`gapp/` (CLI) and `run/` (runtime) are separate Python packages in the same repo. The CLI never imports `gapp_run`. The runtime never imports `gapp`. During `gapp deploy`, the CLI copies `run/` into the Docker build context as `.gapp-run/` and the Dockerfile installs it.
...
The JWT signing key is auto-generated by Terraform (`random_password` + `google_secret_manager_secret`). No manual `gapp secret set` step needed for the framework-level signing key.
...
See the MCP-Auth design doc for full details on credential strategies, caching, and the request validation flow.
...
## Future: MCP Admin Interface

The SDK-first architecture (all logic in `gapp/sdk/`) makes it trivial to expose gapp operations as MCP tools via stdio transport (`gapp mcp`). This would let Claude Code and Gemini CLI drive the full deployment and user management workflow through natural language — no need to learn CLI commands or understand the framework's internals.

Example agent-driven workflow: *"Deploy monarch-access to GCP, pull my current token from ~/.config/monarch/token, and register me as user@example.com."*

The agent would call `gapp_deploy`, `gapp_users_register`, and `gapp_tokens_create` tools in sequence — the same SDK functions the CLI uses today.

Further, the MCP server could include tools that generate client configuration snippets for different agents (Claude Code, Gemini CLI, Claude.ai) — or directly write the configuration for local Linux-based agents.

Not yet implemented. The CLI is the current interface. The MCP layer would be another thin wrapper over the same SDK, following the same pattern as existing tools in the workspace.
